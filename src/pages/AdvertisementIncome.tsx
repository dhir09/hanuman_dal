...
import { Plus, Search, Megaphone, SlidersHorizontal, X } from 'lucide-react'
import { getAllAdvertisementIncomesDesc, getAllEventsDesc, type PaymentMode } from '../db/db'
...
export default function AdvertisementIncome() {
  ...
  const anyFilter = Boolean(modes.length || fromDate || toDate || eventId !== '')
  const activeCount = [modes.length > 0, Boolean(fromDate || toDate), eventId !== ''].filter(Boolean).length

  if (!incomes) return <div className="py-10 text-center text-stone-400">…</div>

  const eventName = (id?: number) => {
    const ev = events?.find((e) => e.id === id)
    return ev ? pick(ev, 'name') : '—'
  }

  const term = q.trim().toLowerCase()
  const list = incomes.filter((inc) => {
    if (term && !(
      inc.advertiser_en.toLowerCase().includes(term) ||
      inc.advertiser_gu.includes(q) ||
      inc.receiptNo.toLowerCase().includes(term) ||
      inc.description_en.toLowerCase().includes(term)
    )) return false
    if (modes.length && !modes.includes(inc.paymentMode)) return false
    if (fromDate && inc.date < fromDate) return false
    if (toDate && inc.date > toDate) return false
    if (eventId !== '' && inc.eventId !== eventId) return false
    return true
  })

  const total = list.reduce((s, inc) => s + inc.amount, 0)

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-stone-800">{t('nav_advertisementIncome')}</h1>
        <Link to="/advertising-income/new" className="btn-primary py-2">
          <Plus size={18} /> {t('add')}
        </Link>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className={`relative shrink-0 rounded-xl p-2.5 ring-1 transition ${
            activeCount ? 'bg-emerald-600 text-white ring-emerald-600' : 'bg-white text-stone-600 ring-stone-200'
          }`}
          aria-label={t('filters')}
        >
          <SlidersHorizontal size={18} />
          {activeCount > 0 && (
            <span className="absolute -right-1.5 -top-1.5 grid h-4 w-4 place-items-center rounded-full bg-white text-[10px] font-bold text-emerald-700 ring-1 ring-emerald-600">
              {activeCount}
            </span>
          )}
        </button>
      ...
      {list.length === 0 ? (
        ...
        <div className="space-y-2">
          {list.map((inc) => (
            <Link key={inc.id} to={`/advertising-income/${inc.id}`} className="card flex items-center justify-between !p-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <Megaphone size={15} className="shrink-0 text-emerald-600" />
                  <div className="truncate text-sm font-semibold text-stone-800">{pick(inc, 'advertiser') || '—'}</div>
                </div>
                <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11px] text-stone-400">
                  <span className="chip bg-stone-100 text-stone-500">{inc.receiptNo}</span>
                  <span>{eventName(inc.eventId)}</span>
                  <span>· {formatDate(inc.date, lang)}</span>
                </div>
              </div>
              <div className="ml-2 text-right">
                <div className="text-sm font-bold tabular-nums text-emerald-600">{formatINR(inc.amount)}</div>
                <div className="mt-0.5 text-[10px] text-stone-400">{t(inc.paymentMode)}</div>
              </div>
            </Link>
          ))}
        </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {PAYMENT_MODES.map((m) => (
                    <button
                      ...
                      className={`chip ${modes.includes(m) ? 'bg-emerald-600 text-white' : 'bg-stone-100 text-stone-600'}`}
                    >
                      {t(m)}
                    </button>
                  ))}
